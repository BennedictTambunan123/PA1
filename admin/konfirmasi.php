...
if(isset($_POST['konfirmasi'])){
   ...
   $status = $_POST['status'];
   $sql = "UPDATE pembayaran SET status='$status' WHERE id_pemesanan=$id";
   $query = mysqli_query($koneksi,$sql);
   if($query){
...
<?php
          ...
          <div class="col-md-4">
            ...
            <div class="form-group">
                <select class="custom-select mt-4" name="status" required="">
                   <option not selected value="">Check Konfirmasi</option>
                     <option value="Pesanan di Konfirmasi">Konfirmasi</option>
                     <option value="Tolak">Tolak</option>

                </select>
                <input type="submit" class="btn btn-warning mt-3" name="konfirmasi" value="Konfirmasi">
            </div>
